...
- **Migración de categoría, dentro de `migrate()`:** cada registro **congela** su `cat` al crearse, y la tarjeta de Registros, el filtro (`r.cat===catFilt`) y el desglose de gasto (`porCat[r.cat]`) usan ese valor congelado — reclasificar la definición no arregla los registros viejos. El remap vive **dentro de `migrate()`**, como paso idempotente **después de `toV2`** (no altera `toV2`): reescribe `cat:"Filtros"→"Fluidos"` **solo** en registros con `itvId==="oil-engine"` (nunca `oil-filter`/"Filtro de aceite"). Por estar en `migrate()` se aplica en los **tres puntos de entrada** (carga, **importar** y semilla), así que un respaldo viejo reimportado queda corregido **en la misma sesión, sin recargar**. Es **silenciosa**, como `toV2` (sin banner; se aplica en cada lectura). Seguimiento no la necesitaba: deriva la categoría del intervalo vinculado, no del `cat` congelado.
  - **Regla general (aprendida de un bug):** primero el remap vivía en el handler de carga (`storageLoad().then`) y se **saltaba al importar** — cualquier respaldo viejo reimportado traía categorías desactualizadas en silencio. **Toda migración de datos va en `migrate()`**, no en un efecto de carga, para que valga en los tres puntos de entrada.
...
- Verificado **en entorno de prueba** (navegador local, ver salvedad abajo): importar un respaldo viejo (Filtros) → Fluidos en la misma sesión sin recargar; carga normal → Fluidos; `toV2` intacto (un registro sin `itvId` se vincula igual); filtro "Filtros" → solo "Filtro de aceite" y "Fluidos" → "Aceite de motor"; desglose agrupa Aceite de motor bajo Fluidos; sin banner; sin errores de consola.

> **⚠ Pendiente conocido, sin resolver — importar respaldo viejo.**
> El fix de **`c4e780a`** (remap dentro de `migrate()`) se verificó en el entorno de prueba, **pero el usuario reportó que el bug persiste en su dispositivo real** tras forzar la actualización: importar un respaldo viejo **todavía muestra "Aceite de motor" en Filtros**, contradiciendo lo verificado en la prueba.
> - **No se confirmó** si es un problema real del fix o un caso de **caché/service worker** que la prueba no capturó (en la verificación local hizo falta desregistrar el SW y limpiar cachés para que corriera el código nuevo — es plausible que el dispositivo siguiera sirviendo una versión vieja pese a "forzar actualización").
> - Se pausó a propósito: el usuario tenía pocos datos guardados y decidió no bloquearse.
> - **Antes de confiar en el flujo de "Importar respaldo" para datos reales, hay que retomar el diagnóstico** (empezar por confirmar qué versión del `index.html` está sirviendo el SW en el dispositivo al momento de importar).
