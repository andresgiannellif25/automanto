...
- Verificado: importar un respaldo viejo (Filtros) → Fluidos en la misma sesión sin recargar; carga normal → Fluidos; `toV2` intacto (un registro sin `itvId` se vincula igual); filtro "Filtros" → solo "Filtro de aceite" y "Fluidos" → "Aceite de motor"; desglose agrupa Aceite de motor bajo Fluidos; sin banner; sin errores de consola.
